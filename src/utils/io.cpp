...
namespace io
{
    ...
    void save_file(std::string write_file_path, DataStructure::string_array outputs)
    {
        ...
        */
        ...
        std::ofstream processed_file[14];
        ...
        processed_file[6].open(write_file_path + "/RETAIL_LIQUIDITY_INDICATOR.csv", std::ios_base::app);
        processed_file[6] << outputs.RETAIL_LIQUIDITY_INDICATOR;

        // OPERATIONAL_HALT_STATUS
        processed_file[7].open(write_file_path + "/OPERATIONAL_HALT_STATUS.csv", std::ios_base::app);
        processed_file[7] << outputs.OPERATIONAL_HALT_STATUS;

        // SYSTEM_EVENT
        processed_file[8].open(write_file_path + "/SYSTEM_EVENT.csv", std::ios_base::app);
        processed_file[8] << outputs.SYSTEM_EVENT;

        // SECURITY_DIRECTORY
        processed_file[9].open(write_file_path + "/SECURITY_DIRECTORY.csv", std::ios_base::app);
        processed_file[9] << outputs.SECURITY_DIRECTORY;

        // SECURITY_EVENT
        processed_file[10].open(write_file_path + "/SECURITY_EVENT.csv", std::ios_base::app);
        processed_file[10] << outputs.SECURITY_EVENT;

        // TRADE_BREAK
        processed_file[11].open(write_file_path + "/TRADE_BREAK.csv", std::ios_base::app);
        processed_file[11] << outputs.TRADE_BREAK;

        // AUCTION
        processed_file[12].open(write_file_path + "/AUCTION.csv", std::ios_base::app);
        processed_file[12] << outputs.AUCTION;

        // UNKOWN_MESSAGE
        processed_file[13].open(write_file_path + "/UNKOWN_MESSAGE.csv", std::ios_base::app);
        processed_file[13] << outputs.UNKNOWN_MESSAGE;
