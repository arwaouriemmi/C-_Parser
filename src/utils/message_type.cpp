...
namespace MessageType
{
    ...
    std::string system_event_message(std::string message)
    {
        ...
        */
        ...
        std::string message_type = "SYSTEM_EVENT"; // Assuming message type is 'S' (0x53)
        std::string system_event = message.substr(2, 2); // Offset 1, length 1
        std::string timestamp = DataType::read_timestamp(message.substr(4, 16));
        if (system_event == "4f")
        {
    ...
    std::string trading_status(std::string message)
    {
        ...
        */
        ...
        else if ((trading_sta == "4f") || (trading_sta == "50"))
        {
            if (trading_sta == "4f")
            {
    ...
    std::string retail_liquidity_indicator(const std::string& message) {
        std::string message_type = "RETAIL_LIQUIDITY_INDICATOR";

        std::string retail_liquidity_indicator_char  = message.substr(2, 2);
        std::string timestamp = DataType::read_timestamp(message.substr(4, 16));
        std::string symbol = DataType::read_string(message.substr(20, 8));
        std::string retail_liquidity_indicator;

  if (retail_liquidity_indicator_char == "20") {
        retail_liquidity_indicator = "Not applicable";
    } else if (retail_liquidity_indicator_char == "41") {
        retail_liquidity_indicator = "Buy interest for Retail";
    } else if (retail_liquidity_indicator_char == "42") { 
        retail_liquidity_indicator = "Sell interest for Retail";
    } else if (retail_liquidity_indicator_char == "43") { 
        retail_liquidity_indicator = "Buy and sell interest for Retail";
    } else {
        retail_liquidity_indicator = "Unknown";
    }
        ...
        std::string output = symbol + delimiter
                             + timestamp + delimiter
                             + retail_liquidity_indicator + newline;
        return output;
    ...
    std::string trade_break_message(std::string message)
    {
        ...
        */
        ...
        std::string output = symbol + delimiter + timestamp + delimiter;
        output += price + delimiter + size + delimiter;
        output +=trade_id + sale_condition_flag +newline;
        return output;
    }
}